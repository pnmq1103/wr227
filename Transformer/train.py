...
def get_ds(config):
    ...
    val_ds = BilingualDataset(valid_ds_raw,tokenizer_src,tokenizer_tar,config["lang_src"],config["lang_tar"],config['seq_len'])
...
def get_model(config, vocab_src_len, vocab_tar_len):
    model = build_transformer(vocab_src_len,vocab_tar_len,config['seq_len'],config['seq_len'],config['d_model'])
    return model
...
def train_model(config): 
    ...
    model = get_model(config, tokenizer_src.get_vocab_size(with_added_tokens=True),tokenizer_tar.get_vocab_size(with_added_tokens=True)).to(device)
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            loss = loss_fn(proj_output.view(-1, tokenizer_tar.get_vocab_size()), label.view(-1))
