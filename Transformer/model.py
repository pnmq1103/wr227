...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, drop_out: float) -> None:
        ...
        self.register_buffer('pe',pe) # save the positional encoding when the model is saved
    def forward(self, x): 
...
class MultiHeadAttentionBlock(nn.Module): 
    ...
    def forward(self, q,  k, v, mask):
        ...
        x = x.transpose(1,2).contiguous().view(x.shape[0],-1, self.h *self.d_k)#(batch, h, seq_len, d_k)-> #(batch,seq_len,h, d_k)
        return self.w_o(x)
    @staticmethod
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, src_mask):
        x = self.residual_connections[0](x,lambda x: self.self_attention_block(x,x,x, src_mask)) # wrap the forward into a one parameter function
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, output, encoded,  src_mask, target_mask): 
        output = self.residual_connection[0](output, lambda x: self.self_attention_block(x,x,x,target_mask))  
        output = self.residual_connection[1](output, lambda x: self.cross_attention_block(x,encoded,encoded,src_mask))
...
class Transformer(nn.Module):
    def __init__(self, encoder : Encoder, decoder: Decoder, src_embed: InputEmbedding, tgt_embedding :InputEmbedding, src_pos: PositionalEncoding, tar_pos :PositionalEncoding, projection :ProjectionLayer):
                super().__init__()
...
def build_transformer(src_vocab_size : int, tar_vocab_size: int, src_seq_len: int, tar_seq_len: int, d_model: int = 512, N: int  =6,  h: int =8, drop_out : float = 0.1, d_ff :int = 2048) ->Transformer: 
    ...
    for _ in range(N):
        ...
        decoder_blocks.append(DecoderBlock(decoder_self_attention,decoder_cross_attention,ffw_2,drop_out))
