...
def gpt(
    ...
    x = rmsnorm(x)  # pre-norm -> reduce "vanishing gradient"
    q: list[Value] = linear(x, state_dict["attn_wq"])
    k: list[Value] = linear(x, state_dict["attn_wk"])
    v: list[Value] = linear(x, state_dict["attn_wv"])
    keys.append(k)  # KV caching
    values.append(v)
    # sotfmax(Q@K^T / sqrt(d)) @ V
    attn_logits: list[Value] = [  # (1xd) x (Nxd)^T
        sum(q[c] * keys[r][c] for c in range(n_embd)) / n_embd**0.5
        for r in range(len(keys))
    ]
    attn_weights: list[Value] = softmax(attn_logits)
    x_attn: list[Value] = [  # (1xN) x (Nxd)
        sum(
            (attn_weights[r] * values[r][c] for r in range(len(values))),
            start=Value(0.0),
        )
