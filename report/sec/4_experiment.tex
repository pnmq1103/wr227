...
\begin{table}[htbp]
  ...
  \label{tab:classification_imagenet100}
\end{table}
...
The inference speed of our model is in in Figure~\ref{fig:inference_speed}
\begin{figure}[htbp]
  \makebox[\linewidth][c]{
    \includegraphics[width=1.325\linewidth]{images/speed.png}
  }
  \caption{The speed and GFLOPS with respect to image size}
...
\subsubsection{Gating Map Visualization}
To qualitatively evaluate the token selection behavior of our MARL framework, we visualize the spatial gating maps generated by the swarm's continuous actions. Figure~\ref{fig:gating_map1s},\ref{fig:gating_map2s} illustrates the original input images alongside their corresponding survival weight heatmaps ($w_{n,l}$) at the deeper layers of the network.
We can observe that the agents successfully learn to distinguish task-salient features from redundant background noise. High survival weight are densely concentrated on the primary subjects. Conversely, the background regions are assigned near-zero weights, silencing them from the recurrent memory update.
...
\begin{figure}[htbp]
  ...
  \caption{}
  \label{fig:gating_map1s}
\end{figure}
\begin{figure}[htbp]
  \centering
  % Adjust the width if it's too large or small
  \includegraphics[width=\linewidth]{images/map2.jpg}
  \caption{}
  \label{fig:gating_map2s}
\end{figure}
