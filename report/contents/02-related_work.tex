...
Linear Attention \cite{shen2021efficient} fixes the quadratic complexity of softmax attention by using kernel functions to approximate the \(Sim()\) operator which reduce the complexity to linear.
...
To reduce the complexity of softmax attention, linear attention replaces softmax operation with a feature map $\phi(\cdot)$
